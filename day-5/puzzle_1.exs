      ...
      [ "seed-to-soil",
        ...
        seed_nb, fn map, loc ->
          ...
          input[map] |> Enum.reduce(%{}, fn cds, cds_dict ->
            ...
            %{ source..(source + (range - 1)) => dest..(dest + (range - 1)) } |> Enum.concat(cds_dict)
            end
          )
          |> Enum.find_value(loc, fn { source, dest } ->
            if loc in source, do: Enum.at(dest, Range.to_list(source) |> Enum.find_index(&(loc === &1)))
          end)
        end
      ) |> min(res)
    end
